...
const Page = ({ stations, email, memberId }: PageProps) => {
  ...
  const onSubmit: SubmitHandler<ReviewProps> = async (data) => {
    ...
    const targetStation = stations.filter(
      (item) => item.stationName === station
    )[0];
    ...
    <div className="flex flex-col w-full pb-10 -mt-2">
      ...
      >
        ...
        <div className="w-full flex flex-col">
          ...
          />
          <div className="text-sm text-red-700 px-2 mt-2">
        ...
        <div>
          <div className="grid grid-cols-3 gap-3 mb-10 px-2">
            {imageUrls.map((url) => (
              <div key={url} className="relative">
                <img
                  src={url}
                  className="shadow rounded h-96 w-96"
                  alt="image preview"
                />
                <div
          ...
          <label
            className={`px-5 py-4 rounded-xl cursor-pointer border ${
              imageUrls.length >= 6
